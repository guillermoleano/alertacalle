...
import { LocationPickerMap } from '@/components/alertacalle/location-picker-map';
...
export default function Reportar() {
    ...
    /* ── Fija coords (mapa / geolocalización) y rellena la dirección ── */
    async function applyCoords({ lat, lng }: { lat: number; lng: number }) {
        setCoords({ lat, lng });
    ...
    /* ── Selección manual en el mapa (click o arrastre del pin) ── */
    function onPickOnMap(c: { lat: number; lng: number }) {
        setGeoStatus('located');
        void applyCoords(c);
    }
                ...
                <div className="mx-auto max-w-2xl">
                    ...
                    <div className="mb-8">
                        ...
                        <p className="mt-2 text-sm leading-6 text-[var(--ac-on-surface-variant)]">
                            Evitá incluir nombres, caras o datos que
                            identifiquen personas.
                        </p>
                    </div>
                            ...
                            <div key={s.n} className="flex flex-1 items-center">
                                <div className="flex flex-col items-center">
                                    <button
                                        type="button"
                                        onClick={() =>
                                            s.n < step && setStep(s.n)
                                        }
                                        className={cn(
                                            'flex size-9 items-center justify-center rounded-full text-[13px] font-bold transition-all duration-200',
                                            step === s.n
                                                ? 'scale-110 bg-[var(--ac-primary)] text-white shadow-md'
                                                : step > s.n
                                                  ? 'cursor-pointer bg-[var(--ac-secondary)] text-white hover:opacity-90'
                                ...
                                {i < steps.length - 1 && (
                                    <div
                                        className={cn(
                                            'mx-1 mb-5 h-0.5 flex-1 transition-all duration-300',
                                            step > s.n
                                                ? 'bg-[var(--ac-secondary)]'
                    ...
                    {/* ── card ── */}
                    <div className="overflow-hidden rounded-2xl border border-[var(--ac-outline-variant)] bg-white shadow-[0_4px_24px_rgba(19,27,46,0.07)]">
                        {/* ── STEP 1: Ubicación ── */}
                        {step === 1 && (
                            <div className="animate-in space-y-5 p-6 duration-250 fade-in slide-in-from-right-4 md:p-8">
                                ...
                                <div className="relative">
                                    ...
                                    <button
                                        ...
                                        className="absolute top-2 left-2 z-10 inline-flex items-center gap-1.5 rounded-lg bg-white/90 px-2.5 py-1.5 text-[11px] font-bold text-[var(--ac-primary)] shadow-sm backdrop-blur-sm transition-all hover:bg-white active:scale-95 disabled:opacity-60"
                                    >
                                        {geoStatus === 'locating' ? (
                                            <>
                                    ...
                                    {coords ? (
                                        <span className="absolute bottom-2 left-2 z-10 inline-flex items-center gap-1 rounded-lg bg-[var(--ac-secondary)] px-2 py-1 text-[10px] font-bold text-white shadow-sm">
                                            <CheckCircle2 className="size-3" />{' '}
                                            Ubicación fijada
                                        </span>
                                    ) : (
                                        <span className="absolute bottom-2 left-2 z-10 inline-flex items-center gap-1 rounded-lg bg-black/55 px-2 py-1 text-[10px] font-bold text-white shadow-sm backdrop-blur-sm">
                                            <MapPin className="size-3" /> Tocá
                                            el mapa para marcar el lugar
                                        </span>
                                    )}
                                    {geoStatus === 'error' && (
                                        <span className="absolute right-2 bottom-2 z-10 max-w-[60%] rounded-lg bg-red-500 px-2 py-1 text-[10px] font-bold text-white shadow-sm">
                                ...
                                <div className="grid gap-4 sm:grid-cols-2">
                                    ...
                                    <label className="grid gap-1.5">
                                        <span className="flex items-center gap-1.5 text-[13px] font-semibold text-[var(--ac-on-surface)]">
                                            <Clock className="size-3.5 text-[var(--ac-primary)]" />{' '}
                                            ¿Cuándo ocurrió?
                                        </span>
                                        <input
                                            type="datetime-local"
                        ...
                        {step === 2 && (
                            <div className="animate-in space-y-5 p-6 duration-250 fade-in slide-in-from-right-4 md:p-8">
                                ...
                                {/* tipo de incidente */}
                                <div>
                                    <p className="mb-3 text-[13px] font-semibold text-[var(--ac-on-surface)]">
                                        Tipo de incidente
                                    </p>
                            ...
                            <div className="animate-in space-y-5 p-6 duration-250 fade-in slide-in-from-right-4 md:p-8">
                                <div>
                                    ...
                                    <p className="mt-1 text-[13px] text-[var(--ac-on-surface-variant)]">
                                        Una foto o video ayuda a validar el
                                        reporte. Máx. 3 archivos, 10 MB c/u.
                                    </p>
                                </div>
                                ...
                                >
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-[11px] text-[var(--ac-on-surface-variant)]">
                                            JPG, PNG, MP4 · Máx. 10 MB ·{' '}
                                            {3 - files.length} restante
                                            {3 - files.length !== 1 ? 's' : ''}
                                        </p>
                                    </div>
                                </button>
                                ...
                                {errors['media.0'] && (
                                    <p className="text-[12px] font-medium text-red-500">
                                        Algún archivo no es válido (formato o
                                        tamaño). Máx. 10 MB, JPG/PNG/MP4.
                                    </p>
                                )}
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {files.map((f, i) => {
                                            ...
                                            return (
                                                <div
                                                    key={i}
                                                    className="group relative aspect-square overflow-hidden rounded-xl border border-[var(--ac-outline-variant)] bg-[var(--ac-surface-container-low)]"
                                                >
                                                    {isVideo ? (
                                                        <div className="flex h-full w-full flex-col items-center justify-center gap-1 text-[var(--ac-secondary)]">
                                                            <Camera className="size-6" />
                                                            <span className="line-clamp-1 px-2 text-center text-[10px] font-semibold text-[var(--ac-on-surface-variant)]">
                                                                {f.name}
                                                            </span>
                                                        </div>
                                                    ) : (
                                ...
                                <div className="flex gap-3 rounded-xl bg-[var(--ac-secondary-fixed)] p-4">
                                    ...
                                    <p className="text-[12px] leading-5 text-[var(--ac-on-secondary-fixed)]">
                                        <strong>
                                            Tu reporte es anónimo por defecto.
                                        </strong>{' '}
                                        Las fotos no incluyen metadatos de
                                        ubicación al ser procesadas. Podés
                                        solicitar borrar tus datos desde Mi
                                        Perfil.
                                    </p>
                                </div>
                            </div>
